<?php
/**
 * Fonction pour obtenir la clé API du vendeur
 * @param string $code_vendeur Code vendeur
 * @return string Clé API ou "zzzz" si code vendeur invalide
 */
function getAPIKey($code_vendeur) {
    // Liste des codes vendeurs valides
    $valid_codes = [
    ...
    // Vérifier si le code vendeur est valide
    if (!in_array($code_vendeur, $valid_codes)) {
        return 'zzzz';
    }
    
    // Générer une clé API factice (15 caractères hexadécimaux)
    return substr(md5($code_vendeur . time()), 0, 15);
}
?> 